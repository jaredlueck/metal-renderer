...
static inline float4 sampleCubeTexture(float3 dir, texturecube<float, access :: read> texture, bool debugFaces = false){
    float x = dir.x;
    float y = dir.y;
    float z = dir.z;
    ...
    return texture.read(uint2(px, py), (uint)index, /*level*/ 0);
}
...
static inline float PCFCube(texturecube<float, access :: read> shadowAtlas,
              sampler shadowSampler,
              float3 dir,
              float receiverDepth,
              float3 normal,
